...
func TestCleanupSiteConnectionsDeletesManagedRemoteResources(t *testing.T) {
	var deletedAccount, deletedKey bool
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodDelete && r.URL.Path == "/api/v1/keys/7":
			deletedKey = true
			writeConnectionTestJSON(w, map[string]any{"success": true})
		default:
	...
	if err := service.CleanupSiteConnections(context.Background(), "user-1", "admin-1", "site-1"); err != nil {
		t.Fatalf("cleanup managed connection: %v", err)
	}
	if !deletedAccount || !deletedKey {
		t.Fatalf("expected remote account and key deletion, account=%t key=%t", deletedAccount, deletedKey)
	}
	if connRepo.connection != nil {
		t.Fatal("expected local managed connection to be deleted")
	}
}

func TestCleanupSiteConnectionsOnlyUnlinksExistingBinding(t *testing.T) {
	connRepo := &testConnRepo{connection: &RealConnection{
		ID: "conn-1", UserID: "user-1", WorkspaceAdminAccountID: "admin-1",
		UpstreamSiteID: "site-1", ProvisioningMode: ProvisioningModeExisting,
	}}
	service := &Service{connRepository: connRepo}

	if err := service.CleanupSiteConnections(context.Background(), "user-1", "admin-1", "site-1"); err != nil {
		t.Fatalf("cleanup existing connection: %v", err)
	}
	if connRepo.connection != nil {
		t.Fatal("expected existing binding to be unlinked locally")
	}
}
