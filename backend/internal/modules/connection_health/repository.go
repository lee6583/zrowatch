...
func (r *Repository) EnsureSchema(ctx context.Context) error {
	statements := []string{
		...
		`CREATE TABLE IF NOT EXISTS connection_health_group_rate_monitor_overrides (
			...
			admin_account_id text NOT NULL DEFAULT '',
			upstream_site_id text NOT NULL,
			upstream_group_key text NOT NULL,
			upstream_group_id text NOT NULL DEFAULT '',
			upstream_group_name text NOT NULL DEFAULT '',
			enabled boolean NOT NULL DEFAULT true,
		...
		`CREATE TABLE IF NOT EXISTS connection_health_group_rate_monitor_states (
			...
			last_probe_at timestamptz NULL,
			updated_at timestamptz NOT NULL DEFAULT now(),
			PRIMARY KEY (user_id, admin_account_id, upstream_site_id, upstream_group_key, target_id)
		)`,
		`CREATE INDEX IF NOT EXISTS idx_group_rate_monitor_states_workspace_group ON connection_health_group_rate_monitor_states (user_id, admin_account_id, upstream_site_id, upstream_group_key)`,
		`CREATE TABLE IF NOT EXISTS connection_health_group_rate_monitor_cycles (
			id text PRIMARY KEY,
			user_id text NOT NULL,
			admin_account_id text NOT NULL DEFAULT '',
			upstream_site_id text NOT NULL,
		...
		`CREATE INDEX IF NOT EXISTS idx_group_rate_monitor_cycles_group_created ON connection_health_group_rate_monitor_cycles (user_id, admin_account_id, upstream_site_id, upstream_group_key, created_at DESC)`,
		`CREATE TABLE IF NOT EXISTS connection_health_group_rate_monitor_actions (
			user_id text NOT NULL,
			admin_account_id text NOT NULL DEFAULT '',
			target_id text NOT NULL,
			account_id text NOT NULL,
			account_name text NOT NULL DEFAULT '',
			upstream_site_id text NOT NULL DEFAULT '',
			upstream_group_key text NOT NULL DEFAULT '',
			original_status text NOT NULL DEFAULT '',
			original_schedulable boolean NOT NULL DEFAULT false,
			last_applied_status text NOT NULL DEFAULT '',
			last_applied_schedulable boolean NOT NULL DEFAULT false,
			pending_status text NOT NULL DEFAULT '',
			pending_schedulable boolean NULL,
			pending_restore boolean NOT NULL DEFAULT false,
			conflict boolean NOT NULL DEFAULT false,
			updated_at timestamptz NOT NULL DEFAULT now(),
			PRIMARY KEY (user_id, admin_account_id, target_id)
		)`,
		`ALTER TABLE connection_health_group_rate_monitor_actions ADD COLUMN IF NOT EXISTS pending_status text NOT NULL DEFAULT ''`,
		`ALTER TABLE connection_health_group_rate_monitor_actions ADD COLUMN IF NOT EXISTS pending_schedulable boolean NULL`,
		`CREATE INDEX IF NOT EXISTS idx_group_rate_monitor_actions_restore ON connection_health_group_rate_monitor_actions (pending_restore, updated_at)`,
