...
type Config struct {
	...
	// 上游站点账号密码加密密钥；未单独配置时兼容复用 SMTPEncryptionKey。
...
func Load() Config {
	...
	return Config{
		...
		UpstreamCredentialEncryptionKey: envOrDefault("UPSTREAM_CREDENTIAL_ENCRYPTION_KEY", os.Getenv("SMTP_ENCRYPTION_KEY")),
	}
}
