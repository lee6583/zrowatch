export default {
  ...
  admin: {
    ...
    userManagement: {
      title: '用户余额管理',
      subtitle: '当前工作区 Sub2API 用户余额',
      total: '共 {count} 位用户',
      loading: '正在加载用户...',
      empty: '没有找到符合条件的用户',
      common: { placeholder: '--', off: '未启用' },
      filters: { search: '搜索邮箱或用户名', allStatuses: '全部状态', allRoles: '全部角色' },
      status: { active: '正常', disabled: '停用' },
      roles: { user: '用户', admin: '管理员' },
      states: { retrying: '充值待重试' },
      table: { user: '用户', status: '状态', balance: '当前余额', warning: '预警阈值', recharge: '自动充值', checkedAt: '最近检测', actions: '操作' },
      dialog: { title: '余额规则', warningEnabled: '余额预警', warningThreshold: '余额预警阈值', rechargeEnabled: '自动充值', rechargeThreshold: '充值触发阈值', rechargeAmount: '每次充值金额' },
      actions: { refresh: '刷新', configure: '配置', save: '保存', cancel: '取消', remove: '取消监控' },
      notices: { saved: '余额规则已保存。', removed: '已取消该用户的余额监控。' },
      errors: { invalidRequest: '余额规则填写不完整或数值无效。', upstreamAuth: '当前 Sub2API 管理会话已失效，请重新登录。', upstreamRequest: 'Sub2API 用户接口请求失败，请稍后重试。', persistence: '余额规则保存失败，请稍后重试。', network: '网络连接失败，请稍后重试。', request: '用户管理请求失败，请稍后重试。' }
    },
    leaderboard: {
      eyebrow: '用量排行',
      title: 'Token 排行榜',
    ...
    connectionHealth: {
      ...
      downstreamConsumption: {
        help: '从对接日起累计统计当前真实对接记录对应的 Sub2API 账号用户实收金额。',
        loading: '正在读取',
        available: '统计完成',
        partial: '部分数据',
        empty: '暂无可统计的下游账号',
        unsupported: '当前下游平台不支持账号消耗统计',
        unavailable: '下游统计不可用',
        accountProgress: '已统计 {success}/{total} 个账号',
        accountConflict: '同一账号关联了多个上游站点，已跳过重复统计',
    ...
    groupRates: {
      ...
      health: {
        ...
        settings: {
          ...
          autoProbe: '自动探活',
          model: '探活模型',
          groupInterval: '探活频率（秒）',
          groupFailureThreshold: '连续失败阈值',
